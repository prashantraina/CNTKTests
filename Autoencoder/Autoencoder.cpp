// Autoencoder.cpp : Defines the entry point for the console application.
//
...
bool LoadMNIST()
{
	...
	CNTK::NDShape sampleShape = { image_height * image_width};
	CNTK::NDShape sequenceShape = { image_height * image_width, 1 };
...
void CreateModel()
{
	...
	encoder = FullyConnectedDNNLayer(inputVar, 400, gpu, CNTK::Sigmoid, L"encoder/hidden1");
	encoder = FullyConnectedDNNLayer(encoder, 10, gpu, CNTK::Sigmoid, L"encoder/output");
	...
	decoder = FullyConnectedDNNLayer(decoderInput, 400, gpu, CNTK::Sigmoid, L"decoder/hidden1");
	decoder = FullyConnectedDNNLayer(decoder, image_height * image_width, gpu, CNTK::Sigmoid, L"decoder/output");
...
void CreateTrainer()
{
	...
	targetVar = CNTK::InputVariable({ image_height * image_width }, CNTK::DataType::Float, L"decoder/target");
...
void DisplayResult()
{
	...
	CNTK::NDShape input_shape = { image_height * image_width };

	CNTK::NDArrayViewPtr input_image = CNTK::MakeSharedObject<CNTK::NDArrayView>
		(CNTK::DataType::Float, input_shape, GetTestingImagePtr(0), image_height * image_width * sizeof(float), cpu);
	...
	cv::Mat inputMat(image_height, image_width, CV_32FC1, GetTestingImagePtr(0));
